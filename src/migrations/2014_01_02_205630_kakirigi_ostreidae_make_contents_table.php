...
class OrchestraStoryMakeContentsTable extends Migration
{
    ...
    public function up()
    {
        $format = Config::get('kakirigi/ostreidae::config.default_format', 'markdown');

        Schema::create('ostreidae_contents', function ($table) use ($format) {
            $table->increments('id');
    ...
    {
        Schema::drop('ostreidae_contents');
    }
}
