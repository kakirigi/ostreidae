...
class Storyteller
{
    ...
    public function url($type, $content = null)
    {
        ...
        if (in_array($type, $predefined)) {
            return handles("kakirigi/ostreidae::{$type}");
        }
    ...
    {
        $format = $this->app['config']->get("kakirigi/ostreidae::permalink.{$type}");

        if (is_null($format) or ! ($content instanceof Content)) {
            return handles("kakirigi/ostreidae::/");
        }
        ...
        return handles("kakirigi/ostreidae::{$format}");
    }
}
